...
export class DoctorService {
  ...
  addDoctor(doctor: Doctor): Observable<any> {
    return this.http.post(`${API_URL}doctor`, doctor);
  }
  ...
  searchDoctors(searchTerm: string): Doctor[] {
    ...
    return allDoctors.filter(
      (doctor) =>
        ...
        (doctor.practice && doctor.practice.toLowerCase().includes(term)) ||
  ...
  getDoctorStats(): DoctorStats {
    ...
    const totalPatients = doctors.reduce(
      (total, doctor) => total + (doctor.patients ? doctor.patients.length : 0),
      0
    );
    ...
    const recentWork = doctors.reduce((total, doctor) => {
      ...
      return (
        total +
        doctor.patients.filter((patient) => {
